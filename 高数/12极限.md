...
- [极限](#极限)
  - [数列的极限](#数列的极限)
    ...
    - [常用的数列极限](#常用的数列极限)
  - [函数的极限](#函数的极限)
    - [当$x \\to \\infty$时，函数f(x)的极限](#当x-to-infty时函数fx的极限)
    - [当$x \\to x\_0$时，函数f(x)的极限](#当x-to-x_0时函数fx的极限)
    - [函数极限的性质](#函数极限的性质)
      - [函数极限的唯一性](#函数极限的唯一性)
      - [函数极限的局部有界性](#函数极限的局部有界性)
      - [函数极限的局部保号性](#函数极限的局部保号性)
  - [无穷小与无穷大](#无穷小与无穷大)
    - [无穷小比较](#无穷小比较)
    - [等价无穷小替换定理](#等价无穷小替换定理)
  - [极限的四则运算法则](#极限的四则运算法则)
  ...
  - [两个重要极限](#两个重要极限)
    ...
    - [第二个重要极限](#第二个重要极限)
  - [考点](#考点)
    - [函数极限和数列极限的求解](#函数极限和数列极限的求解)
      - [利用分解因式或分子（分母）有理化求极限](#利用分解因式或分子分母有理化求极限)
      - [利用$\\infty \\over \\infty$型有理分式结论求极限](#利用infty-over-infty型有理分式结论求极限)
      - [利用无穷小的性质求极限](#利用无穷小的性质求极限)
      - [利用等价无穷小替换求极限](#利用等价无穷小替换求极限)
      - [利用两个重要极限求极限](#利用两个重要极限求极限)
      - [利用数列求和求极限](#利用数列求和求极限)
      - [利用夹逼准则求极限](#利用夹逼准则求极限)
    - [求分段函数的极限](#求分段函数的极限)
    - [求极限中的参数](#求极限中的参数)
...
若数列{$x_n$}收敛，则数列{$x_n$}是有界数列.

#### 数列极限的保号性

若$\lim_{n \to \infty}x_n=A,且A>0(或A<0)$，则存在正整数N，使当n>N时，有$x_n>0(或x_n<0)$

#### 性质4

若$\lim_{n \to \infty}x_n=A，则对{x_n}的任何子数列{x_{k_n}},有\lim_{k \to \infty}x_{n_k}=A$

### 常用的数列极限

1. 等比数列$1,q,q^2,\cdots,q^n,\cdots(|q|<1)$的极限是0，即$\lim_{n \to \infty}q^n(|q|<1)$;
2. 当$\alpha>0时,\lim_{n \to \infty}\frac{1}{n^\alpha}=0$;
3. $lim_{n \to \infty}\sqrt[n]{a}=1(a>0)$;
4. $lim_{n \to \infty}\sqrt[n]{n}=1$.

## 函数的极限

### 当$x \to \infty$时，函数f(x)的极限

$\lim{x \to \infty}f(x)=A$的充分必要条件是$\lim{x \to -\infty}f(x)=\lim{x \to +\infty}f(x)A$

### 当$x \to x_0$时，函数f(x)的极限

$\lim{x \to x_0}f(x)=A$的充分必要条件是$\lim{x \to x_0^-}f(x)=\lim{x \to x_0^+}f(x)A$
...
若极限$\lim_{x \to x_0}f(x)$存在，则在点$x_0$的某个去心邻域内，函数$f(x)$有界.

#### 函数极限的局部保号性

若$\lim_{n \to x_0}f(x)=A,且A>0(或A<0)$，在点$x_0$的某个去心邻域内，有$f(x)>0(或f(x)<0)$
